...
3. **「等级 N」标签随滑杆手柄移动**（每级 55.3px）。先用宽带 ROI 定位标签，再取它右侧的窄条读数字。
   ★★ **字形集必须覆盖 0~9。** 滑杆上限随游戏进程增长（实测 8 → 10），到 10 就出现两位数；
   2026-09-18 真机故障：`dig_panel_level` 只有 1~8，缺 0/9 → 「10」读成「16」被阈值拒 → `readSliderLevel` 返回 null
   → 对账式的 `setSearchFloor` 算不出差值、一次都没点 → 滑杆永远停在 10 → 附近只有 7/8 级的点，永远搜不到
