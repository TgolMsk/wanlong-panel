  ...
  return (
    ...
    >
      ...
      <Alert
        ...
        message="只在模板全不命中时才问，只会点「关闭 / 取消」，并把认出的关闭按钮自学成模板"
        description={
          <div className="wla-form-note">
            采集与采样流程认不出当前界面时（多半是活动弹窗盖着），把截图发给视觉模型，
            让它判断界面并从白名单里选动作：<b>点关闭按钮</b>、<b>点「取消」</b>、按返回、不动。
            只有前两种会被执行，点完复验画面确实变了才算数；「确定」「派兵」这类动作永远不在名单里。
            关掉弹窗后回到已知界面，就把点击前那一帧里的关闭按钮裁成模板存进模板库 —— 下次同样的弹窗
            1 毫秒本地解决，不再问。正常挂机一天可能一次都不触发，
            截图里含账号信息，会发到你填的接口。
          </div>
        }
      />
